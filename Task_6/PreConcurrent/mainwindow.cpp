...
void MainWindow::StartRace(void){
    ...
    if(ui->rb_qtConcur->isChecked()){
        ...
        bool mutexOn = true;
        futRace1 = QtConcurrent::run([this, initNum, mutexOn]() {concurRace1->DoWork(&number, mutexOn, initNum);});
        futRace2 = QtConcurrent::run([this, initNum, mutexOn]() {concurRace2->DoWork(&number, mutexOn, initNum);});
        futRace1.waitForFinished();
        futRace2.waitForFinished();
        //futRace1 = QtConcurrent::run(concurRace1, &ExampleRace::DoWork, &number, ui->rb_mutexOn->isChecked(), ui->sb_initNum->value());
